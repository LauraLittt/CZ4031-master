...
int BPTree::removeKey(int x) {
	Node* root = getRoot();
	int numOfNodeRemoved = 0;

	// If tree is empty
	if (root == NULL) {
		cout << "B+ Tree is Empty" << endl;
		return numOfNodeRemoved;
	}
	...
	if (leftSibling >= 0) {// If left sibling exists
		...
		cout << "Merging two leaf Nodes" << endl;
		numOfNodeRemoved += removeInternal(parent->keys[rightSibling - 1], parent, rightNode);//delete parent Node Key
		//delete rightNode;
		numOfNodeRemoved++;
	}
	return numOfNodeRemoved;
